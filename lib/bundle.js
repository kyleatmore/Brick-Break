...
var Brick = function () {
  function Brick(xPos, yPos, hitsToDestroy, color, ctx) {
...
var Game = function () {
  ...
  _createClass(Game, [{
    ...
    value: function populateBricks() {
      ...
      for (var row = 0; row < 5; row++) {
        for (var col = 0; col < 13; col++) {
          var brick = new _brick2.default(xPos, yPos, 1, this.colors[row], this.ctx);
    ...
    key: 'moveBall',
    value: function moveBall(timeDelta) {
      this.ball.bounce();
...
var Ball = function (_MovingObject) {
  ...
  _createClass(Ball, [{
    ...
    value: function collideWith(otherObject) {
      if (this.xPos + this.width <= otherObject.xPos || this.xPos >= otherObject.xPos + otherObject.width) {
        this.bounceX();
      } else {
        this.bounceY();
      }
